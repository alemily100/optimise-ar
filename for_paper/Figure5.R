...
fm1 <- lmer(adjusted_value ~  timepoint + factor(dose) -1 + (1 | id), diff, na.action = na.omit)
tidy_model <- broom.mixed::tidy(fm1, effects = "fixed", conf.level=0.95, conf.int = TRUE)

summary<-summary%>%mutate(pred = case_when(
  dose == 1 ~ tidy_model$estimate[1]*timepoint+tidy_model$estimate[2],
  dose == 2 ~ tidy_model$estimate[1]*timepoint+tidy_model$estimate[3],
  dose == 3 ~ tidy_model$estimate[1]*timepoint+tidy_model$estimate[4]
))
